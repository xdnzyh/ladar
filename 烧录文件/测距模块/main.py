...
LOOP_SLEEP_MS = 1
CCD_DRAIN_BUDGET_US = 5000
CCD_DRAIN_MAX_READS = 32
AUX_DIAGNOSTIC_DELAY_US = 500000
...
class MeasurementNode:
    def __init__(self):
        ...
        self.tx_queue = []
        self.aux_low_since = None
        self.aux_warning_sent = False
    ...
    def flush_tx(self):
        ...
        if not self.aux.value():
            now = self.clock.now()
            if self.aux_low_since is None:
                self.aux_low_since = now
            elif (not self.aux_warning_sent and
                  now - self.aux_low_since >= AUX_DIAGNOSTIC_DELAY_US):
                self.usb_print("LORA AUX LOW; TX QUEUE BLOCKED")
                self.aux_warning_sent = True
            return
        self.aux_low_since = None
        self.aux_warning_sent = False
    ...
    def drain_ccd(self):
        ...
        reads = 0
        while self.ccd.any() and reads < CCD_DRAIN_MAX_READS:
            if self.clock.now() - started >= CCD_DRAIN_BUDGET_US:
                break
            available = self.ccd.any()
            if not available:
                break
            self.ccd.read(min(256, available))
            reads += 1

    def apply_fixed_exposure(self):
        """Always program the fixed exposure index. No readback exists in the guide."""
    ...
    def poll_ccd(self, now):
        if self.capture is not None and now - self.capture["begin"] > CCD_TIMEOUT_US:
    ...
    def start_scan(self, source, session, rate_hz, requested_exposure, mode):
        if not session or mode not in ("fffe", "raw2"):
            ...
            return
        if self.ccd_quarantine:
            self.respond(source, "ERROR CCD_QUARANTINED")
            return
        ...
        # exposure is deliberately fixed at 5 for the current calibration.
    ...
    def start_calibration(self, source, token, requested_exposure, mode):
        if (self.scan_active or self.capture is not None or self.batch is not None
                or self.cal_pending is not None or self.ccd_quarantine):
        ...
        self.cal_restore_laser = self.laser.value()
        self.laser.value(1)
    ...
    def start_manual_measure(self, source, mode="fffe"):
        if self.scan_active or self.cal_pending is not None or self.batch is not None:
            ...
            return
        if self.ccd_quarantine:
            self.respond(source, "ERR CCD QUARANTINED; REBOOT REQUIRED")
            return
        if self.capture is not None:
    ...
    def status(self, source):
        self.respond(
            source,
            (
                "STATUS {} LASER={} EXPOSURE_FIXED={} CCD_QUARANTINED={} SCAN={} SESSION={} "
                "MODE={} RATE_HZ={:.3f} SEQ={} TXQ={} AUX={} DEBUG={} INPUT={} "
                "SCAN_ATTEMPTS={} PIX_SENT={} CCD_TIMEOUTS={} INVALID_FRAMES={}"
            ).format(
                VERSION,
                self.laser.value(),
                EXPOSURE_INDEX,
                int(self.ccd_quarantine),
    ...
    def handle_command(self, raw, source):
        ...
        # shape but intentionally program exposure 5 regardless.
        if (len(upper) == 8 and upper.startswith("@C") and
            ...
            if 0 <= value <= 13:
                ...
                if value != EXPOSURE_INDEX:
                    self.usb_print(
                        "LEGACY requested exposure {}; forced to 5".format(value)
                    )
                if self.apply_fixed_exposure():
