...
// Read-only link probe. Stream the payload instead of allocating a long SRAM buffer.
static void linkProbe(const char *tag, uint8_t length) {
  char header[32];
  snprintf_P(header,sizeof(header),PSTR("@CFG,T,%s,%u,"),tag,(unsigned int)length);
  uint16_t crc=textCrc(header,(uint8_t)strlen(header));
  Serial.print(header);
  for(uint8_t i=0;i<length;i++) {
    char digit='0'+i%10;
    Serial.print(digit); crc=crcByte(crc,(uint8_t)digit);
  }
  char tail[8]; snprintf_P(tail,sizeof(tail),PSTR(",%04X"),(unsigned int)crc);
  Serial.println(tail);
}
static bool process(char *line, bool running) {
  ...
  *last=0;
  ...
  if(op=='T') {
    if(open) {error(nonce,2);return true;}
    if(count!=4 || !number(fields[3],arg) || arg<1 || arg>128) {error(nonce,3);return true;}
    linkProbe(nonce,(uint8_t)arg);return true;
  }
  if(op=='I' && count==3) {
    char body[48];
